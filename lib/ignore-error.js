...
function isErrorInTemplateBlock(error, templateContent, coordinates) {
  ...
  // Check if the error is within the template block's line range
  if (error.line < coordinates.start.line || error.line > coordinates.end.line) {
    return false;
  }

  // If the error is on the same line as the start or end, check the column
  if (error.line === coordinates.start.line && error.column < coordinates.start.column) {
    return false;
  }
  if (error.line === coordinates.end.line && error.column > coordinates.end.column) {
    return false;
  }
